...
describe 'Knife::Windows::Core msi download functionality for knife Windows winrm bootstrap template' do
  ...
  before(:all) do
    ...
    # the downloaded msi
    @local_omnibus_root = "#{ENV['SYSTEM_DRIVE']}/opscode/chef"
  ...
  describe "running on any version of the Windows OS", :windows_only, :appveyor_only do
  ...
  def install_succeeded?
    Dir.exists?(@local_omnibus_root)
  end
  ...
  def run_download_scenario
    ...
    expect(install_succeeded?).to be true
  end
end
